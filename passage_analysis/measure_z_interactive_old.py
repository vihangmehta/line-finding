...
s3_9532_vac, he10830_vac, pg_10941_vac, pb_12822_vac, pa_18756_vac]
...
'[S III]', 'He I', r'P$\gamma$', r'P$\beta$', r'P$\alpha$']
...
def getzeroorders(zeroorderpath, g='G141', magcut=23.5):  # MB: changed from 23.0
    ...
    zmag = np.array(zmag)
    cond = (zmag <= magcut)
    t = Table([zox[cond], zoy[cond], zoid[cond], zora[cond], zodec[cond]], names=('x', 'y', 'objid'))
    return t

def getzeroorders_from_cat(zeroorderpath, g='G141', magcut=23.5): #KN: updated function to use phot catalog
    if (verbose == True): print('Running getzeroorders_from_cat...\n') # MDR 2022/05/17
    """
    Changing to return a table
    """
    zeroorder = Table.read(zeroorderpath)
    t = Table([zeroorder['x'], zeroorder['y'], zeroorder['id'],zeroorder['ra'], zeroorder['dec'], zeroorder['mag_auto']], names=('x', 'y', 'objid','ra','dec','mag_auto'))
    return t
...
def inspect_object(user, par, obj, objinfo, lamlines_found, ston_found, g102zeros, g141zeros, linelistoutfile, commentsfile, remaining, allobjects, show_dispersed=True, stored_fits = False, path_to_wisp_data = ' '):
    ...
    if path_to_wisp_data ==  ' ':
        print('HERE:   Par%i_'+str(obj).zfill(5)+'.G115_1D.dat' % (par))
        specnameg102 = 'Par%i_'+str(obj).zfill(5)+'.G115_1D.dat' % (par) # specnameg102 = 'Par%i_G102_BEAM_%iA.dat' % (par, obj) - M.D.R. - 10/08/2020 ... From e.g. Par406_G141_BEAM_422A.dat
        specnameg141 = 'Par%i_'+str(obj).zfill(5)+'.G150_1D.dat' % (par) # specnameg141 = 'Par%i_G141_BEAM_%iA.dat' % (par, obj) - M.D.R. - 10/08/2020 ... To e.g. MU1_295_G141.1D.dat
    else :
        print('HERE: ', path_to_wisp_data + 'Par' + str(par) + '/Spectra/Par'+str(par)+'_'+str(obj).zfill(5)+'.G115_1D.dat')
        specnameg102 = path_to_wisp_data + 'Par' +str(par)+ '/Spectra/Par'+str(par)+'_'+str(obj).zfill(5)+'.G115_1D.dat'
        specnameg141 = path_to_wisp_data + 'Par' +str(par)+ '/Spectra/Par'+str(par)+'_'+str(obj).zfill(5)+'.G150_1D.dat'
    ...
    # display the object
  #  print('What is this?????: ', g141zeros)
    if g102zeros is not None:
        #show2dNEW('G102', par, obj, g102firsts, g102zeros, 'linear')
        show2dNEW('F115W', par, obj, g102zeros, user, 'linear', path_to_wisp_data = path_to_wisp_data)
    if g141zeros is not None:
        show2dNEW('F150W', par, obj, g141zeros, user, 'linear', path_to_wisp_data = path_to_wisp_data)
    if g141zeros is not None:
        show2dNEW('F200W', par, obj, g141zeros, user, 'linear', path_to_wisp_data = path_to_wisp_data)  
...
#        print('No previous fit results found. Fitting this object now.')
    ...
    # KVN 2023/09/01 - Changed first guess to OIII for PASSAGE
...
### replace this with printouts from pickle files
    ...
    's3_9532':0, 'he10830':0, 'pg_10941':0, 'pb_12822':0, 'pa_18756':0,'cont':0} # MDR 2022/07/22
...
#        print "    Guess Redshift: z = %f" % (zguess)
        ...
        # REMOVE any of the nodes used for the continuum spline by wavelength
            ...
            strnw = ','.join(str(nw) for nw in config_pars['node_wave'])
            print_prompt("Enter wavelength of the nodes you would like to REMOVE: w1, w2, w3, ...")
            print_prompt("current node wavelengths are: %s" % strnw)
            nodestr = input("> ")
            nodesplit = nodestr.split(',')
            node_arr_rem = []
            ...
            unmatched_nodes = [k for k in node_arr_rem if k not in config_pars['node_wave']]
        ...
        # ADD nodes used for the continuum spline
            ...
            strnw = ','.join(str(nw) for nw in config_pars['node_wave'])
            upper_wav_bound = config_pars['lambda_max'] ; lower_wav_bound = config_pars['lambda_min']
            print_prompt("Enter wavelength of the nodes you would like to ADD: w1, w2, w3, ...")
            print_prompt("current node wavelengths are: %s" % strnw)
            nodestr = input("> ")
            nodesplit = nodestr.split(',')
            node_arr_add = []
            ...
            try:
                ...
                print_prompt('Invalid entry, possibly because. Enter wavelengths separated by commas')
            node_arr_add = np.array(node_arr_add)
            # remove any added nodes that are outside the blue/red cutoff
            node_goodadd = [k for k in node_arr_add if k <= upper_wav_bound and k >= lower_wav_bound]
            if len(node_goodadd) != len(node_arr_add):
                print_prompt('At least one of the nodes is outside the wavelength range and has not been added.')
        ...
        # SHIFT ALL nodes used for the continuum spline by some wavelength
            ...
            node_good = [k for k in node_arr if k <= upper_wav_bound and k >= lower_wav_bound]
            if len(node_good) != len(node_arr):
                print_prompt('At least one of the nodes is outside the wavelength range and is removed.')
        ...
        # SHIFT INDIVIDUAL nodes used for the continuum spline by some wavelength
            ...
            except ValueError: print_prompt('Invalid entry. Enter wavelengths separated by commas')
            print_prompt("Enter wavelength by which you would like to SHIFT this node: w1")
            try:
                shift_wav=float(input("> "))
            except ValueError: print_prompt('Invalid entry. Enter single wavelength')
        ...
        # change 2d stamp scaling to log
            ...
            if g102zeros is not None:
                show2dNEW('F115W', par, obj, g102zeros, user, 'log', path_to_wisp_data = path_to_wisp_data )
            if g141zeros is not None:
                show2dNEW('F150W', par, obj, g141zeros, user, 'log', path_to_wisp_data = path_to_wisp_data)
        ...
        # change g102 2d stamp scaling to zscale
            ...
            try:
                ...
                if g102zeros is not None:
                    show2dNEW('F115W', par, obj, g102zeros, user, 'linear',
        ...
        # change g141 2d stamp scaling to zscale
            ...
            try:
                ...
                if g141zeros is not None:
                    show2dNEW('F150W', par, obj, g141zeros, user, 'linear',
                              zran1=z1, zran2=z2, path_to_wisp_data = path_to_wisp_Data )
        ...
        # reload full iamges
            ...
            showDirectNEW(obj, par, g141zeros, load_image=True, path_to_wisp_data = path_to_wisp_data)
            if show_dispersed:
...
#     g141firstordreg = path_to_wisp_data + '/Par' + str(par) + '/DATA/DIRECT_GRISM/G141_1st.reg'
    ...
    secats = glob(os.getcwd()+'/DATA/DIRECT_GRISM/Par*phot*.fits') # MDR 2022/05/17
    secats.sort()
    ...
    if os.path.exists(g102zeroordreg[0]):
        ...
        show2dNEW('F115W', parnos[0], objid_unique[0], g102zeroarr, user, 'linear', path_to_wisp_data = path_to_wisp_data)
    else:
        g102zeroarr = None
        g102firstarr = None
    if os.path.exists(g141zeroordreg[0]):
        g141zeroarr = getzeroorders_from_cat(g141zeroordreg[0], g='G102')
        show2dNEW('F150W', parnos[0], objid_unique[0], g141zeroarr, user, 'linear', path_to_wisp_data = path_to_wisp_data)
    else:
        g141zeroarr = None
        g141firstarr = None
    if os.path.exists(f200zeroordreg[0]):
        f200zeroarr = getzeroorders_from_cat(f200zeroordreg[0], g='G102')
        show2dNEW('F200W', parnos[0], objid_unique[0], f200zeroarr, user, 'linear', path_to_wisp_data = path_to_wisp_data)
    else:
        f200zeroarr = None
        f200firstarr = None
        
    showDirectNEW(objid_unique[0], parnos[0], g102zeroarr, load_image=True, path_to_wisp_data = path_to_wisp_data)
#     if show_dispersed:  # MB
#         showDispersed(objid_unique[0], parnos[0], load_image=True, path_to_wisp_data  = path_to_wisp_data)
    ...
    while remaining_objects.shape[0] > 0:
        ...
        if (use_stored_fits == True):
            ### get pickle files:
            inpickles = []
            path_pickle1 = path_to_stored_fits + '/Par'  + str(parnos[0]) + '_output_a/fitdata/Par0_' + str(next_obj) + '_fitspec.pickle'
